...
(function ($, window, document, undefined) {
  ...
  $.fn.calendar = function (parameters) {
    ...
    $allModules.each(function () {
      ...
      module = {
        ...
        update: {
          focus: function focus(updateRange, container) {
        ...
        get: {
          ...
          calendarModule: function calendarModule(selector) {
            if (!selector) {
              return null;
            }
        ...
        set: {
          date: function date(_date, updateInput, fireChange) {
            updateInput = updateInput !== false;
            fireChange = fireChange !== false;
            _date = module.helper.sanitiseDate(_date);
            _date = module.helper.dateInRange(_date);

            var mode = module.get.mode();
            var text = formatter.datetime(_date, settings);
            if (fireChange && settings.onChange.call(element, _date, text, mode) === false) {
              return false;
            }

            module.set.focusDate(_date);

            if (settings.isDisabled(_date, mode)) {
              return false;
            }

            var endDate = module.get.endDate();
            if (!!endDate && !!_date && _date > endDate) {
              //selected date is greater than end date in range, so clear end date
              module.set.endDate(undefined);
            }
            module.set.dataKeyValue(metadata.date, _date);

            if (updateInput && $input.length) {
              $input.val(text);
            }
          },
          startDate: function startDate(date, refreshCalendar) {
          ...
          endDate: function endDate(date, refreshCalendar) {
            date = module.helper.sanitiseDate(date);
            var endModule = module.get.calendarModule(settings.endCalendar);
            if (endModule) {
          ...
          focusDate: function focusDate(date, refreshCalendar, updateFocus, updateRange) {
            date = module.helper.sanitiseDate(date);
          ...
          monthOffset: function monthOffset(_monthOffset, refreshCalendar) {
            var multiMonth = Math.max(settings.multiMonth, 1);
            _monthOffset = Math.max(1 - multiMonth, Math.min(0, _monthOffset));
            module.set.dataKeyValue(metadata.monthOffset, _monthOffset, refreshCalendar);
          },
          mode: function mode(_mode, refreshCalendar) {
            module.set.dataKeyValue(metadata.mode, _mode, refreshCalendar);
          },
          dataKeyValue: function dataKeyValue(key, value, refreshCalendar) {
        ...
        selectDate: function selectDate(date, forceSet) {
          var mode = module.get.mode();
        ...
        helper: {
          sanitiseDate: function sanitiseDate(date) {
            if (!date) {
              return undefined;
            }
          ...
          dateDiff: function dateDiff(date1, date2, mode) {
            mode = mode || 'day';
            var isTimeOnly = settings.type === 'time';
            var isYear = mode === 'year';
            ...
            date1 = new Date(isTimeOnly ? 2000 : date1.getFullYear(), isTimeOnly ? 0 : isYear ? 0 : date1.getMonth(), isTimeOnly ? 1 : isYearOrMonth ? 1 : date1.getDate(), !isHourOrMinute ? 0 : date1.getHours(), !isMinute ? 0 : 5 * Math.floor(date1.getMinutes() / 5));
            date2 = new Date(isTimeOnly ? 2000 : date2.getFullYear(), isTimeOnly ? 0 : isYear ? 0 : date2.getMonth(), isTimeOnly ? 1 : isYearOrMonth ? 1 : date2.getDate(), !isHourOrMinute ? 0 : date2.getHours(), !isMinute ? 0 : 5 * Math.floor(date2.getMinutes() / 5));
            return date2.getTime() - date1.getTime();
          },
          dateEqual: function dateEqual(date1, date2, mode) {
            return !!date1 && !!date2 && module.helper.dateDiff(date1, date2, mode) === 0;
          },
          isDateInRange: function isDateInRange(date, mode, minDate, maxDate) {
            if (!minDate && !maxDate) {
              var startDate = module.get.startDate();
          ...
          dateInRange: function dateInRange(date, minDate, maxDate) {
            if (!minDate && !maxDate) {
              var startDate = module.get.startDate();
              minDate = startDate && settings.minDate ? new Date(Math.max(startDate, settings.minDate)) : startDate || settings.minDate;
              maxDate = settings.maxDate;
          ...
          mergeDateTime: function mergeDateTime(date, time) {
  ...
  $.fn.calendar.settings = {
    ...
    formatter: {
      header: function header(date, mode, settings) {
      ...
      monthHeader: function monthHeader(date, settings) {
      ...
      hourHeader: function hourHeader(date, settings) {
        return settings.formatter.date(date, settings);
      },
      minuteHeader: function minuteHeader(date, settings) {
        return settings.formatter.date(date, settings);
      },
      dayColumnHeader: function dayColumnHeader(day, settings) {
      ...
      date: function date(_date2, settings) {
        ...
        var month = settings.text.months[_date2.getMonth()];
      ...
      cell: function cell(_cell, date, cellOptions) {}
    },
    ...
    isDisabled: function isDisabled(date, mode) {
      return false;
    },
