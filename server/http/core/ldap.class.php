...
class BackupLDAP
{
	...
	public function importUsers() {
		...
		do {
			...
			$query =
				"REPLACE INTO backup_user (user_id, user_name,first_name,last_name,email,address,city,state,zip,title,office,mobile,source) " .
				"VALUES (UNHEX(?),?,?,?,?,?,?,?,?,?,?,?,?)";
			...
			if ( $stmt = mysqli_prepare($this->_dbconn, $query) ) {
				...
				for ( $i=0; $i < $userTotal; $i++ ) {
					...
					$generatedUserId = sha1( $entries[$i]['samaccountname'][0] . ":" . $entries[$i]['mail'][0] );
					...
					if ( $stmt->execute() ) {
						...
						//Get user object
						$user = BackupUser::getUser($generatedUserId);
