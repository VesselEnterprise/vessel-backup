...
class BackupUpload
{
	...
	**/
	private function _getUpload($fileId) {
		...
		$query = "SELECT a.*,COUNT(b.upload_id) AS parts_uploaded FROM backup_upload AS a LEFT JOIN backup_upload_part AS b ON a.upload_id=b.upload_id WHERE a.file_id=?";
	...
	public function initUpload() {
		...
		if ( $this->_isIgnoreType($fileType) ) {
			$this->_setError("File type " . $fileType . " is not allowed to upload");
			return false;
		}

		//The database file ID is the SHA-1 hash of the file path
		...
		//Insert a new file
		$query = "REPLACE INTO backup_file (file_id,file_name,file_size,user_id,file_type,hash,file_path,compressed,last_modified,last_backup) VALUES(?,?,?,?,?,?,?,?,FROM_UNIXTIME(?),NOW())";
		if ( $stmt = mysqli_prepare($this->_dbconn, $query ) ) {
	...
	*/
	public function uploadPart($contents) {
		...
		if ( !$this->_uploadRow ) {
			$this->_setError("There is no pending upload for file id " . $this->_metadata->{'file_id'} . " does not exist");
			return false;
		}
	...
	*/
	public function completeUpload($uploadId=-1) {
		...
		if ( $uploadId > 0 && !$this->_uploadRow ) {
			...
			$this->_file = new BackupFile($this->_fileId, $this->_userId);
		}
	...
	**/
	private function _setFileUploaded($fileId) {
		...
		$query = "UPDATE backup_file SET uploaded=1 WHERE file_id=? AND user_id=?";
		if ( $stmt = mysqli_prepare($this->_dbconn, $query) ) {
