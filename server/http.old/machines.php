...
if ( !isset($_GET['action']) && $common['session']->isLoggedIn() ) {
  ...
  //Get a count of total users for pagination
	$result = mysqli_query($common['dbconn'], "SELECT COUNT(*) FROM backup_machine");
	...
	//Query results
	$users=array();
	$query = "SELECT * FROM backup_machine ORDER BY ? LIMIT ?,?";
	...
	$vars = array("machines" => $users, "totalPages" => $totalPages);
	$renderer->addTemplate('machine_list', $vars);
