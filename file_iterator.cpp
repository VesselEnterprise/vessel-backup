...
FileIterator::FileIterator(const std::string& path) : m_base_path(path), m_itr(m_base_path), m_skip_dir_periods(false)
{
...
void FileIterator::scan()
{
    ...
    const fs::directory_entry& current_entry = *m_itr;

    if ( !fs::exists(current_entry.path()) ) {
        cout << "Error: Could not scan " <<  current_entry.path().string() << ". Directory does not exist" << '\n';
        return;
    }
