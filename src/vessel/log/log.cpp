...
Log::Log() : m_logger(keywords::channel = LOG_FILENAME)
{

    m_log_path = AppManager::get().get_data_dir() + PATH_SEPARATOR() + "logs";

    logging::register_simple_formatter_factory< Vessel::Logging::severity_level, char >("Severity");

    logging::add_file_log
    (
        keywords::file_name = m_log_path + PATH_SEPARATOR() + LOG_FILENAME + "_%Y%m%d.log",
        ...
        keywords::filter = channel == LOG_FILENAME
    );
...
void Log::add_message(const std::string& msg, const std::string& type )
{
    ...
    if ( m_sql_logging && LocalDatabase::is_open() ) add_sql_message(msg, type);
}
...
void Log::add_error(const std::string& msg, const std::string& type )
{
    ...
    if ( m_sql_logging && LocalDatabase::is_open() ) add_sql_message(msg, type, true);
}
...
void Log::add_exception(const std::exception& ex)
{
    ...
    if ( m_sql_logging && LocalDatabase::is_open() ) add_sql_exception(ex);
}

void Log::add_file_exception(const std::exception& ex)
{
    BOOST_LOG_SEV( m_logger, static_cast<Vessel::Logging::severity_level>(error) ) << logging::add_value(category, "") << ex.what();
}
